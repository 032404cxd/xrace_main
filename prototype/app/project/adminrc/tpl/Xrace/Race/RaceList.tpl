...
<script type="text/javascript">
  ...
  function RaceDelete(r_id, r_name){
    deleteRaceBox = divBox.confirmBox({content:'是否删除 ' + r_name + '?<p>关联的报名记录和计时点配置将同时删除',ok:function(){location.href = '{tpl:$this.sign/}&ac=race.delete&RaceId=' + r_id;}});
  }
  ...
  function RaceResultUpdate(r_id){
      RaceResultUpdateBox = divBox.confirmBox({content:'是否更新比赛记录?<p>将于几分钟内由定时任务更新',ok:function(){location.href = '{tpl:$this.sign/}&ac=race.result.update&RaceId=' + r_id;}});
  }

...
class XraceTeamController extends AbstractController
{
    ...
    {
        //获取 页面参数
        $bind=$this->request->from('TeamName','TeamComment','CreateUserId','IsTemp','RaceStageId');
        //队伍名称不能为空
        ...
        {
            //检测更新用户的信息
            $CreateUserInfo = $this->oUser->getUser($bind['CreateUserId'],"UserId,UserName");
            //如果用户信息合法
    ...
    {
        //获取 页面参数
        $bind=$this->request->from('TeamId','TeamName','TeamComment','UpdateUserId');
        //检测队伍Id
        if($bind['TeamId'])
        {
            //队伍名称不能为空

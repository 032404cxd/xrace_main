...
<table class="table table-bordered table-striped" width="100%">
		...
		<th><label for="parent">父级菜单</label></th><td>
			...
			<?php if (is_array($root)) { foreach ($root as $row) { ?>            	
                ...
                <?php } else if ($MenuCount != 1) { ?>
                    <option value="<?php echo $row['menu_id']; ?>" selected ><?php echo $row['name']; ?></option>
                <?php } ?>
			<?php } } ?>
			</select>		
        </td>
	...
	<tr>
		...
		<textarea name="permission_list" id="permission_list" class="span5" rows="4"><?php echo $menu['permission_list']; ?></textarea><td>*</td>
	</tr>
	<tr>
		<th>&nbsp;</th><td>

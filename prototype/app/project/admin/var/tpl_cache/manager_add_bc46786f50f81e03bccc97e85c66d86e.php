...
<input type="text" name="password" id="password" class="span4" value="<?php echo $pass; ?>" /></td></tr>
<tr>
<th><label for="confirm">密码确认</label></th><td>
<input type="text" name="confirm" id="confirm" class="span4" value="<?php echo $pass; ?>" /></td></tr>
<tr>
  <th>用户分类</th>
...
<tr>
	...
	<?php if (is_array($menuGroup)) { foreach ($menuGroup as $row) { ?><option value="<?php echo $row['group_id']; ?>"><?php echo $row['name']; ?></option><?php } } ?>
	</select>	
	</td>
</tr>
<tr>
	...
	<?php if (is_array($dataGroup)) { foreach ($dataGroup as $row) { ?><input type="checkbox" name="data_groups[]" id="data_groups[]" value="<?php echo $row['group_id']; ?>" /><?php echo $row['name']; ?>&nbsp;<?php } } ?></td>
</tr>

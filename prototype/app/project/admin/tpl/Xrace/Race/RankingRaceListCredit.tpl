      ...
      {tpl:if($RaceStageInfo.comment.RaceStructure=="race")}
        ...
        <tr>
          ...
          <td align="center" class="rowtip" /}><input type="checkbox" name="RaceList[{tpl:$RaceId/}][{tpl:$RaceGroupId/}][selected]" value="1" {tpl:if($RaceGroupInfo.selected > 0)}checked{/tpl:if} /></td>
            ...
            <select name="RaceList[{tpl:$RaceId/}][{tpl:$RaceGroupId/}][RankingTypeId]" class="span2" size="1">
                {tpl:loop $CreditList $CreditId $CreditInfo}
                <option value="{tpl:$CreditId/}" {tpl:if($RaceGroupInfo.selected == $CreditId)}selected="selected"{/tpl:if}>{tpl:$CreditInfo.CreditName/}</option>
                {/tpl:loop}
                </td>
        </tr>
              {/tpl:loop}
      ...
      {tpl:else}
        ...
        <tr>
          ...
          <td align="center" class="rowtip" /}><input type="checkbox" name="RaceList[{tpl:$RaceId/}][{tpl:$RaceGroupId/}][selected]" value="1" {tpl:if($RaceInfo.selected >0 )}checked{/tpl:if} /></td>
            ...
            <select name="RaceList[{tpl:$RaceId/}][{tpl:$RaceGroupId/}][RankingTypeId]" class="span2" size="1">
                {tpl:loop $CreditList $CreditId $CreditInfo}
                <option value="{tpl:$CreditId/}" {tpl:if($RaceInfo.selected==$CreditId)}selected="selected"{/tpl:if}>{tpl:$CreditInfo.CreditName/}</option>
                {/tpl:loop}
                </td>
